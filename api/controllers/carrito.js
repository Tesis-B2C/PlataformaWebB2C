...
async function getCarrito(req, res) {
    ...
    try {
        if (!verificar) {
            ...
            let carritoObtenido = await Carrito.findOne({where: {COD_AGENTE: busqueda}, include: {model: Carrito_Producto, include:{model:Producto, include:[{model:Variante,include:{model:Imagen_Producto}},{model:Producto_Descuento, include: {model:Descuento}}]}}});
            ...
            if (carritoObtenido) {
                ...
                res.status(404).send({
                    message: 'No existe carrito de compras registrado en la base de datos'
                });
...
async function saveCarrito(req, res){
    ...
    try {
        if (!verificar) {
            ...
            let carritoGuardado = await Carrito_Producto.create({ID_PRODUCTO:req.body.ID_PRODUCTO,COD_PRODUCTO:req.body.COD_PRODUCTO,ID_CARRITO:verificar.CARRITO.ID_cARRITO});
